...
public class ParticleEffectsLibrary : MonoBehaviour {
	...
	void Awake () {
		...
		if (ParticleEffectSpawnOffsets.Length != TotalEffects) {
			///1///Debug.LogError ("ParticleEffectsLibrary-ParticleEffectSpawnOffset: Not all arrays match length, double check counts.");
		}
		if (ParticleEffectPrefabs.Length != TotalEffects) {
			///1///Debug.LogError ("ParticleEffectsLibrary-ParticleEffectPrefabs: Not all arrays match length, double check counts.");
		}
